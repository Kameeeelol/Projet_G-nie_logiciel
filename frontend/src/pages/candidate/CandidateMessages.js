...
export default function CandidateMessages() {
  const { user, axiosAuth, API, token } = useAuth();
  ...
  const fetchConversations = useCallback(async () => {
    try {
      const res = await axiosAuth().get("/conversations");
  ...
  const fetchMessages = useCallback(async (convoId) => {
    try {
      const res = await axiosAuth().get(`/messages/${convoId}`);
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex gap-4 animate-fade-in" data-testid="candidate-messages">
      {/* Conversation list */}
          ...
          <>
            <div className="p-4 border-b border-slate-800 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">{activeConvo.other_user?.company_name || activeConvo.other_user?.role}</p>
              </div>
            </div>
              ...
              <div className="space-y-3">
                {messages.map(msg => (
                  <div key={msg.id} className={`flex ${msg.sender_id === user?.id ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[70%] rounded-lg px-3 py-2 ${msg.sender_id === user?.id ? "bg-indigo-500 text-white" : "bg-slate-800 text-slate-200"}`}>
                      <p className="text-sm">{msg.content}</p>
                      <p className={`text-[10px] mt-1 ${msg.sender_id === user?.id ? "text-indigo-200" : "text-slate-500"}`}>
            ...
            <div className="p-4 border-t border-slate-800">
              <form onSubmit={e => { e.preventDefault(); handleSend(); }} className="flex gap-2">
                <Input
                  ...
                  className="flex-1 bg-slate-950 border-slate-800 focus:ring-1 focus:ring-indigo-500 h-10 text-slate-200 placeholder:text-slate-600"
                />
                <Button data-testid="send-message-btn" type="submit" disabled={sending || !newMsg.trim()} className="bg-indigo-500 hover:bg-indigo-600 text-white h-10 px-4">
